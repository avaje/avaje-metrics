...
public class VirtualMachineMetrics {
  ...
  public static class GarbageCollectorStats {
    ...
    /**
     * Returns the amount of time in the given unit the garbage collector has
     * taken in total.
     * 
     * @param unit
  ...
  VirtualMachineMetrics(MemoryMXBean memory, List<MemoryPoolMXBean> memoryPools,
      OperatingSystemMXBean os, ThreadMXBean threads,
      List<GarbageCollectorMXBean> garbageCollectors, RuntimeMXBean runtime, MBeanServer mBeanServer) {
    this.memory = memory;
   ...
   * 
   * @return total Heap and non-heap memory currently committed to the JVM in
   *         bytes.
   */
  ...
  /**
   * Returns the percentage of the JVM's non-heap memory (e.g., direct buffers)
   * which is being used.
   * 
  ...
  /**
   * Returns a map of memory pool names to the percentage of that pool which is
   * being used.
   * 
   * @return a map of memory pool names to the percentage of that pool which is
   *         being used
   */
  public Map<String, Double> getMemoryPoolUsage() {
    final Map<String, Double> pools = new TreeMap<String, Double>();
    for (MemoryPoolMXBean pool : memoryPools) {
      final double max = pool.getUsage().getMax() == -1 ? pool.getUsage().getCommitted() : pool
          .getUsage().getMax();
  ...
  /**
   * Returns the percentage of available file descriptors which are currently in
   * use.
   * 
   * @return the percentage of available file descriptors which are currently in
   *         use, or {@code NaN} if the running JVM does not have access to this
   *         information
   */
  public double getFileDescriptorUsage() {
    try {
      final Method getOpenFileDescriptorCount = os.getClass().getDeclaredMethod(
          "getOpenFileDescriptorCount");
      getOpenFileDescriptorCount.setAccessible(true);
      final Long openFds = (Long) getOpenFileDescriptorCount.invoke(os);
      final Method getMaxFileDescriptorCount = os.getClass().getDeclaredMethod(
   ...
   * @see <a href="http://java.sun.com/j2se/versioning_naming.html">J2SE SDK/JRE
   *      Version String Naming Convention</a>
   */
   ...
   * 
   * @return the name of the currently-running jvm, eg
   *         "Java HotSpot(TM) Client VM"
   * @see <a
   *      href="http://download.oracle.com/javase/6/docs/api/java/lang/System.html#getProperties()">System.getProperties()</a>
   */
  ...
  /**
   * Returns the number of live threads (includes {@link #getDaemonThreadCount}.
   * 
  ...
  public Map<String, GarbageCollectorStats> getGarbageCollectors() {
    ...
    for (GarbageCollectorMXBean gc : garbageCollectors) {
      stats.put(gc.getName(),
          new GarbageCollectorStats(gc.getCollectionCount(), gc.getCollectionTime()));
    }
  ...
  /**
   * Returns a map of thread states to the percentage of all threads which are
   * in that state.
   * 
  ...
  public void getThreadDump(OutputStream out) {
    ...
    for (int ti = threads.length - 1; ti >= 0; ti--) {
      ...
      if (lock != null && t.getThreadState() != Thread.State.BLOCKED) {
        writer.printf("\n    - waiting on <0x%08x> (a %s)", lock.getIdentityHashCode(),
            lock.getClassName());
        writer.printf("\n    - locked <0x%08x> (a %s)", lock.getIdentityHashCode(),
            lock.getClassName());
      } else if (lock != null && t.getThreadState() == Thread.State.BLOCKED) {
        writer.printf("\n    - waiting to lock <0x%08x> (a %s)", lock.getIdentityHashCode(),
            lock.getClassName());
      }
  ...
  public Map<String, BufferPoolStats> getBufferPoolStats() {
    try {
      ...
      final BufferPoolStats directStats = new BufferPoolStats(
          (Long) ((Attribute) directAttributes.get(0)).getValue(),
          (Long) ((Attribute) directAttributes.get(1)).getValue(),
          (Long) ((Attribute) directAttributes.get(2)).getValue());

      stats.put("direct", directStats);

      final BufferPoolStats mappedStats = new BufferPoolStats(
          (Long) ((Attribute) mappedAttributes.get(0)).getValue(),
